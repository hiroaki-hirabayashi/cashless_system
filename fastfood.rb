...
def eat_in
  ...
  if    command >= 2
    puts <<~text
    ポイントは#{(selected_menu.get_total_price_10(count)*0.02).to_i}円分(5%分)を還元します！
    ありがとうございました！
    text
  elsif  command == 1
    loop do
...
def takeout
  ...
  if    command >= 2
    puts <<~text
    ポイントは#{(selected_menu.get_total_price_8(count)*0.02).to_i}円分(5%分)を還元します！
    ありがとうございました！
    text
  elsif  command == 1
    loop do
